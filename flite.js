...
function detectFeatures(cb) {
  var usage = /usage/i;
  child.exec('/app/.heroku/vendor/bin/flite --version', function (err, stdout) {
    dep.flite = /flite-1\.4/.test(stdout);
    if (!dep.flite) {
      dep.init = true;
      return cb(new Error('required binary flite not available'));
    }
    child.exec('/app/.heroku/vendor/bin/flite -lv', function (err, stdout) {
...
function tmp(cb) {
  temp.open('/app/.heroku/vendor/bin/flite-tmp', function (err, file) {
    if (err) return cb(err);
...
function cmd(also) {
  var cmdStr = '/app/.heroku/vendor/bin/flite ';
